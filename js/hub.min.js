...
".toggle-comments",function(){a(this).closest("li").find(".h-comments").slideToggle()});a(".h-saved .h-close-saved").on("click",function(){a(this).closest(".h-saved").slideUp();return!1})},initBulkActions:function(){function b(g,l,k){if(g&&l&&g[0]&&l[0]&&!g.is(l[0])){var n=!1;l.parent().children().each(function(t,u){if(n){if(g.is(u)||l.is(u))return!1;a(u).find("input:checkbox.js-bulk-mode:visible").prop("checked",k).change()}else if(g.is(u)||l.is(u))n=!0})}}var c=this;this.$bulk_menu=a("ul.js-bulk-menu:first");
...
c.$bulk_menu.children("li").toggle();return!1});this.$bulk_menu.find("a.js-bulk-action").click(function(){var g=a(this);if(c.bulkCount()){var l=g.data("confirm");if(!l||confirm(l))l=g.data("action"),l="bulk"+l.substr(0,1).toUpperCase()+l.substr(1),c[l+"Action"].apply(c,[g])}else alert($_("Select at least one topic"));return!1});this.$topics_ul.on("change",":input.js-bulk-mode",function(){c.bulkCount();this.checked?a(this).closest("li").addClass("selected").find("h3").addClass("bold"):a(this).closest("li").removeClass("selected").find("h3").removeClass("bold")});
var d=null,f=null;this.$topics_ul.on("click",":input.js-bulk-mode",function(g){var l=a(this),k=l.closest(".h-topic");l.prop("checked")?(g.shiftKey&&d&&b(d,k,!0),d=k,f=null):(g.shiftKey&&f&&b(f,k,!1),d=null,f=k);c.bulkCount()});a.storage.get("sort-handler-unavailable-notice-hidden")?a(".sort-handler-unavailable-notice").remove():a(".sort-handler-unavailable-notice .h-close").click(function(){a(this).closest(".sort-handler-unavailable-notice").remove();a.storage.set("sort-handler-unavailable-notice-hidden",
!0);return!1})},initFollowLinks:function(){this.$topics_ul.on("click",".h-follow",function(){var b=a(this),c=b.find("i"),d=c.hasClass("star-empty")?1:0;a.post("?module=following",{topic_id:b.data("topic"),follow:d},function(f){if("ok"==f.status){f=parseInt(a("#following-count").html());var g=b.closest("li"),l=g.find(".h-comments");d?(l.slideDown(),g.addClass("h-followed").removeClass("h-not-followed"),b.addClass("highlighted"),c.removeClass("star-empty").addClass("star"),b.find(".followers-count").html($_("Following")),
f+=1):(l.slideUp(),g.removeClass("h-followed").addClass("h-not-followed"),b.removeClass("highlighted"),c.addClass("star-empty").removeClass("star"),b.find(".followers-count").html(""),--f);a("#following-count").addClass("highlighted").html(f)}},"json");return!1})},initTopicTypesFilter:function(){function b(){var l={};g.map(function(){if(this.checked)return 1;l[this.value]=1}).length||(l={});var k=[],n=[];d.children().each(function(){var t=a(this).data("type-id");l[t]?k.push(this):n.push(this)});k.length?
(a(k).css("opacity",.7).slideUp(function(){a(k).css("opacity","");a(window).scroll()}),a(".h-footer .place-for-hidden-label").show().children("span").text(k.length),d.closest(".h-stream").addClass("h-js-filtered").removeClass("h-not-js-filtered")):(a(".h-footer .place-for-hidden-label").hide().children("span").text("0"),d.closest(".h-stream").addClass("h-not-js-filtered").removeClass("h-js-filtered"));n.length&&a(n).slideDown(function(){a(window).scroll()})}function c(){var l=g.map(function(){return(this.checked||
null)&&a.trim(a(this).closest("label").text())}).get().join(", ");l=a.trim(f.find("ul li.selected a").text())+(l?": "+l:"");f.find("> li > a i").text(l)}var d=a("#h-content .h-topics"),f=a(".h-sort.js-sort-menu"),g=f.find(".h-filter-by-type :checkbox");f.find(".h-filter-by-type").on("change",":checkbox",function(){b();c()});d.on("lazyload_append",function(){b()})},initManualDragAndDrop:function(b){this.$topics_ul.sortable({items:"> li",distance:5,tolerance:"pointer",handle:".sort,h3 a,i.h-glyph32",
